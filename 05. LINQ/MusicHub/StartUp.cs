        ...
        public static string ExportAlbumsInfo(MusicHubDbContext context, int producerId)
        {
            ...
            foreach (var album in albumsInfo)
            {
                sb.AppendLine($"-AlbumName: {album.Name}")
                    ...
                    .AppendLine($"-Songs:");
                int songNumber = 1;
                foreach (var songInAlbum in album.Songs)
                {
                    sb.AppendLine($"---#{songNumber}")
                      .AppendLine($"---SongName: {songInAlbum.SongName}")
                      .AppendLine($"---Price: {songInAlbum.Price:F2}")
                      .AppendLine($"---Writer: {songInAlbum.Writer}");
                    songNumber++;
                }
